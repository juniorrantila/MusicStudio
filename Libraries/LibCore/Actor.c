...
C_API bool actor_init_reloadable(Actor* actor, FSVolume* volume, FileID library, c_string dispatch_name)
{
    VERIFY(actor);
    VERIFY(volume);
    ...
    if (!handle) {
        errorf("could not open library '%s'", path);
        return false;
    }

    void(*dispatch)(void) = dlsym(handle, dispatch_name);
    if (!dispatch) {
        errorf("could not find '%s'", dispatch_name);
        return false;
    }
...
{
    for (u64 i = 0; i < ARRAY_SIZE(actor->handles); i++) {
...
C_API bool actor_reload(Actor* actor)
{
    ...
    if (fs_file_needs_reload(file) && !fs_file_reload(file)) {
        errorf("could not reload actor file");
        return false;
    }
    ...
    if (!handle) {
        errorf("could not open library '%.*s'", path_size, path);
        return false;
    }
    ...
    if (!dispatch) {
        errorf("actor '%.*s' does not contain symbol '%s'", path_size, path, symbol);
        return false;
    }

    u64 old_index = actor->current_handle;
    u64 new_index = (old_index + 1) % ARRAY_SIZE(actor->handles);
