...
C_API void memory_poker_init(MemoryPoker* poker)
{
    VERIFY(!ty_is_initialized(poker));
    defer [=] { ty_set_initialized(poker); };
    memzero(poker, sizeof(*poker));
}

[[nodiscard]] bool MemoryPoker::start() { return memory_poker_start(this); }
C_API [[nodiscard]] bool memory_poker_start(MemoryPoker* poker)
{
    pthread_t thread;
...
{
    init_default_context("memory-poker");
