...
@interface UIAppKitWindow : NSWindow<NSWindowDelegate> {
    ...
    @public void(*display_callback)(UIWindow*, void* user);

    @public void* scroll_user;
    @public void(*scroll_callback)(UIWindow*, void* user);

    @public v2 scroll_delta;
...
int ui_window_hide(UIWindow* win)
{
    auto* window = (__bridge UIAppKitWindow*)win;
...
v2 ui_window_size(UIWindow* win)
{
    auto* window = (__bridge UIAppKitWindow*)win;
    return (v2){(f32)window->size.width, (f32)window->size.height};
}
...
v2 ui_window_mouse_pos(UIWindow* win)
{
    auto* window = (__bridge UIAppKitWindow*)win;
    v2 window_size = ui_window_size(win);

    auto mousePosition = [window convertPointFromScreen:[NSEvent mouseLocation]];
    return (v2){(f32)mousePosition.x, (f32)(window_size.y - mousePosition.y)};
}
...
v2 ui_window_scroll_delta(UIWindow* win)
{
    auto* window = (__bridge UIAppKitWindow*)win;
...
void ui_window_set_size(UIWindow* window, v2 size)
{
    auto* win = (__bridge UIAppKitWindow const*)window;
...
-(void)flagsChanged:(NSEvent *)event
{
    ...
    // auto alert = [NSAlert alertWithError:[NSError errorWithDomain:@"foobar" code:2 userInfo:@{
    //     NSLocalizedDescriptionKey: NSLocalizedString(@"some description", nil),
    //     NSLocalizedFailureReasonErrorKey: NSLocalizedString(@"Some failutre reason", nil),
    //     NSLocalizedRecoverySuggestionErrorKey: NSLocalizedString(@"Some suggestion", nil)
    // }]];
    // auto res = alert.runModal;
    // NSLog(@"modal response: %@", @(res));
    // [alert beginSheetModalForWindow:[NSApp mainWindow] completionHandler:^(NSModalResponse response) {
    //     NSLog(@"modal response %@", @(response));
    // }];
...
-(void)scrollWheel:(NSEvent *)event {
    self->scroll_delta = v2f(
        (f32)event.scrollingDeltaX,
