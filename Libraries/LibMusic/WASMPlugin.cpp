...
#include <LibTy/Verify.h>
#include <LibTy/Assert.h>
...
WASMPlugin::WASMPlugin(WASMPluginManager* manager, StringView name, IM3Environment env, IM3Runtime runtime, IM3Module mod)
    : manager(manager)
    , fallback_name(name)
    , env(env)
    , runtime(runtime)
...
ErrorOr<WASMPlugin> WASMPlugin::create(WASMPluginManager* manager, StringView name, Bytes bytes)
{
    ...
    IM3Module mod;
    if (c_string res = m3_ParseModule(env, &mod, bytes.items, bytes.count)) {
