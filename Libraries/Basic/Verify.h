...
#define DEBUG_ASSERT(__expression)                      \
    do {                                                \
        if (!(__expression)) { \
            __builtin_printf(VERIFY_RED "ASSERT" VERIFY_NORMAL ": %s: \"" VERIFY_CYAN "%s" VERIFY_NORMAL "\" " VERIFY_RED "failed" VERIFY_NORMAL " [%s:%d]\n", __FUNCTION__, #__expression, __FILE__, __LINE__);  \
            __builtin_trap();                           \
        }                                               \
    } while (0)
#endif
...
#ifndef guard
#define guard(__expression)                 \
    if (({                                  \
        bool __result = (__expression);    \
        if (!__result) __builtin_printf(VERIFY_RED "GUARD" VERIFY_NORMAL ": %s: \"" VERIFY_CYAN "%s" VERIFY_NORMAL "\" " VERIFY_RED "failed" VERIFY_NORMAL " [%s:%d]\n", __FUNCTION__, #__expression, __FILE__, __LINE__);  \
        __result; \
    })) {}
#endif
