...
ByteFound ByteDecoder::peek_string(u64 count, StringSlice* out) const { return byte_peek_string(this, count, out); }
C_API ByteFound byte_peek_string(ByteDecoder const* decoder, u64 count, StringSlice* out)
{
    Bytes bytes = {};
...
C_API ByteFound byte_peek_u16le(ByteDecoder const* decoder, u16* out)
{
    ...
    if (out) *out = ty_device_endian_from_u16le(value);
    return byte_found();
}
...
C_API ByteFound byte_peek_u32le(ByteDecoder const* decoder, u32* out)
{
    ...
    if (out) *out = ty_device_endian_from_u32le(value);
    return byte_found();
}
...
C_API ByteFound byte_peek_u64le(ByteDecoder const* decoder, u64* out)
{
    ...
    if (out) *out = ty_device_endian_from_u64le(value);
    return byte_found();
}
...
C_API ByteFound byte_peek_u16be(ByteDecoder const* decoder, u16* out)
{
    ...
    if (bytes.count != sizeof(u16))
        return byte_not_found();
    u16 value; memcpy(&value, bytes.items, sizeof(value));
    if (out) *out = ty_device_endian_from_u16be(value);
    return byte_found();
}
...
C_API ByteFound byte_peek_u32be(ByteDecoder const* decoder, u32* out)
{
    ...
    if (bytes.count != sizeof(u32))
        return byte_not_found();
    u32 value; memcpy(&value, bytes.items, sizeof(value));
    if (out) *out = ty_device_endian_from_u32be(value);
    return byte_found();
}
...
C_API ByteFound byte_peek_u64be(ByteDecoder const* decoder, u64* out)
{
    ...
    if (bytes.count != sizeof(u64))
        return byte_not_found();
    u64 value; memcpy(&value, bytes.items, sizeof(value));
    if (out) *out = ty_device_endian_from_u64be(value);
    return byte_found();
}

ByteFound ByteDecoder::peek_ileb128(i64* out) const { return byte_peek_ileb128(this, out); }
C_API ByteFound byte_peek_ileb128(ByteDecoder const* decoder, i64* out)
{
    u64 current_cursor = decoder->cursor;
    auto get_next_byte_in_input = [&](u8* out) -> ByteFound {
        if (current_cursor >= decoder->bytes.count)
            return byte_not_found();
        *out = decoder->bytes.items[current_cursor++];
        return byte_found();
    };
    ...
    u64 size = sizeof(result) * 8; // no. of bits in signed integer

    // will be assigned inside the do-while loop, but referenced afterwards
    u8 byte = 0;

    do {
        if (!get_next_byte_in_input(&byte).found)
            return byte_not_found();
...
ByteFound ByteDecoder::peek_uleb128(u64* out) const { return byte_peek_uleb128(this, out); }
C_API ByteFound byte_peek_uleb128(ByteDecoder const* decoder, u64* out)
{
    u64 current_cursor = decoder->cursor;
    auto get_next_byte_in_input = [&](u8* out) -> ByteFound {
        if (current_cursor >= decoder->bytes.count)
...
ByteExpect ByteDecoder::expect(StringSlice value) { return byte_expect_string(this, value); }
C_API ByteExpect byte_expect_string(ByteDecoder* decoder, StringSlice value)
{
...
ByteExpect ByteDecoder::expect_ileb128(i64 value) { return byte_expect_ileb128(this, value); }
C_API ByteExpect byte_expect_ileb128(ByteDecoder* decoder, i64 value)
{
    i64 result = 0;
    if (!byte_peek_ileb128(decoder, &result).found)
        return byte_error();
    if (result != value)
        return byte_error();
    VERIFY(byte_parse_ileb128(decoder, &result).found);
    return byte_ok();
}

ByteExpect ByteDecoder::expect_uleb128(u64 value) { return byte_expect_uleb128(this, value); }
C_API ByteExpect byte_expect_uleb128(ByteDecoder* decoder, u64 value)
{
    u64 result = 0;
    if (!byte_peek_uleb128(decoder, &result).found)
        return byte_error();
    if (result != value)
        return byte_error();
    VERIFY(byte_parse_uleb128(decoder, &result).found);
    return byte_ok();
}

ByteFound ByteDecoder::parse_string(u64 count, StringSlice* out) { return byte_parse_string(this, count, out); }
C_API ByteFound byte_parse_string(ByteDecoder* decoder, u64 count, StringSlice* out)
{
...
ByteFound ByteDecoder::parse_ileb128(i64* out) { return byte_parse_ileb128(this, out); }
C_API ByteFound byte_parse_ileb128(ByteDecoder* decoder, i64* out) 
{
    u64 current_cursor = decoder->cursor;
    auto get_next_byte_in_input = [&](u8* out) -> ByteFound {
        if (current_cursor >= decoder->bytes.count)
    ...
    // the size in bits of the result variable, e.g., 64 if result's type is int64_t
    u64 size = sizeof(result); // no. of bits in signed integer
    ...
    decoder->cursor = current_cursor;
    if (out) *out = result;
    return byte_found();
}

ByteFound ByteDecoder::parse_uleb128(u64* out) { return byte_parse_uleb128(this, out); }
C_API ByteFound byte_parse_uleb128(ByteDecoder* decoder, u64* out) 
{
    u64 current_cursor = decoder->cursor;
    auto get_next_byte_in_input = [&](u8* out) -> ByteFound {
        if (current_cursor >= decoder->bytes.count)
