...
static bool binary_to_compressed_c(char const* filename, char const* symbol, char const* output_header_filename, char const* output_source_filename);

int main(int argc, char** argv)
{
    if (argc < 5)
    {
        printf("USAGE: %s <input-file> <symbol-name> <output-header> <output-source>\n", argv[0]);
        return -1;
    }
    auto filename = argv[1];
    auto symbol_name = argv[2];
    auto output_header_filename = argv[3];
    auto output_source_filename = argv[4];

    printf("%s %s %s %s %s\n", argv[0], filename, symbol_name, output_header_filename, output_source_filename);

    bool ret = binary_to_compressed_c(filename, symbol_name, output_header_filename, output_source_filename);
    if (!ret)
...
bool binary_to_compressed_c(const char* filename, const char* symbol,
    char const* output_header_filename, char const* output_source_filename)
{
    // Read file
    ...
    FILE* output_header = fopen(output_header_filename, "w+");
    fprintf(output_header, "#pragma once\n\nnamespace Fonts {\n\nextern const char %s[%d+1];\n\n}", symbol, (int)((compressed_sz + 3) / 4)*5);

    // Output as Base85 encoded
    FILE* output_source = fopen(output_source_filename, "w+");
    fprintf(output_source, "\nnamespace Fonts {\n\n");
    fprintf(output_source, "extern const char %s[%d+1] =\n    \"", symbol, (int)((compressed_sz + 3) / 4)*5);
    char prev_c = 0;
    for (int src_i = 0; src_i < compressed_sz; src_i += 4)
    {
        ...
        for (unsigned int n5 = 0; n5 < 5; n5++, d /= 85)
        {
            ...
            fprintf(output_source, (c == '?' && prev_c == '?') ? "\\%c" : "%c", c);
            prev_c = c;
        }
        if ((src_i % 112) == 112 - 4)
            fprintf(output_source, "\"\n    \"");
