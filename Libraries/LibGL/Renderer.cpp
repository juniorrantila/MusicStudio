...
static bool compile_shader(GLenum shader_type, char const* source, u32 source_len, GLuint* out);
static bool link_program(GLuint vert, GLuint frag, GLuint* out);
...
GLShaderID GLRenderer::shader(GLShaderSource source) { return gl_shader(this, source); }
C_API GLShaderID gl_shader(GLRenderer* r, GLShaderSource source)
{
    if (!render_init(r))
    ...
    *shader = (GLShader){
    ...
    GLuint vertex;
    if (!compile_shader(GL_VERTEX_SHADER, source.vert.items, source.vert.count, &vertex))
        return gl_shader_id_null;
    defer [&] { glDeleteShader(vertex); };

    GLuint fragment;
    if (!compile_shader(GL_FRAGMENT_SHADER, source.frag.items, source.frag.count, &fragment))
        return gl_shader_id_null;
    defer [&] { glDeleteShader(fragment); };

    if (!link_program(vertex, fragment, &shader->program))
        return gl_shader_id_null;
...
static bool render_init(GLRenderer* r)
{
    ...
    for (u32 i = 0; i < ARRAY_SIZE(gl_vertex_attribs); i++) {
...
static bool compile_shader(GLenum shader_type, char const* source, u32 source_len, GLuint* out)
{
    ...
    if (!compiled) {
        ...
        errorf("could not compile shader: %.*s", message_size - 1, message);
        return false;
    }
...
static bool link_program(GLuint vert, GLuint frag, GLuint* out)
{
    ...
    if (!linked) {
        ...
        errorf("could not link: %.*s", message_size, message);
