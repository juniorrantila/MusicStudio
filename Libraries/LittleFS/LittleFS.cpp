...
static int lfs_bd_cmp(lfs_t *lfs,
    ...
    const uint8_t *data = (uint8_t const*)buffer;
    lfs_size_t diff = 0;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        while (true) {
            ...
            if ((fmask & tag) == (fmask & ftag)) {
                struct lfs_diskoff diskoff = {
                    dir->pair[0], (lfs_off_t)(off+sizeof(tag))
                };
                int res = cb(data, tag, &diskoff);
                if (res < 0) {
...
nextname:
        ...
        *path = name;
        ...
        while (true) {
            struct lfs_dir_find_match match = {
                lfs, name, namelen
            };
    ...
    while (true) {
        {
            ...
            struct lfs_dir_commit_commit commit_commit = {
                lfs, &commit
            };
            ...
            err = lfs_dir_traverse(lfs,
                    ...
                    begin, end, -begin,
                    lfs_dir_commit_commit, &commit_commit);
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
                    goto relocate;
                }
...
static int lfs_ctz_find(lfs_t *lfs,
    ...
    lfs_off_t loff = size - 1;
    lfs_off_t current = lfs_ctz_index(lfs, &loff);
...
static int lfs_ctz_traverse(lfs_t *lfs,
    ...
    lfs_off_t loff = size - 1;
    lfs_off_t index = lfs_ctz_index(lfs, &loff);
...
static lfs_ssize_t lfs_file_write_(lfs_t *lfs, lfs_file_t *file,
    ...
    if (!(file->flags & LFS_F_WRITING) && file->pos > file->ctz.size) {
        ...
        while (file->pos < pos) {
            uint8_t buffer = 0;
            lfs_ssize_t res = lfs_file_flushedwrite(lfs, file, &buffer, 1);
            if (res < 0) {
                return res;
            }
        }
...
static lfs_soff_t lfs_file_seek_(lfs_t *lfs, lfs_file_t *file,
    ...
    if (
        ...
        lfs_off_t pos = file->pos;
        int oindex = lfs_ctz_index(lfs, &pos);
        lfs_off_t noff = npos;
...
static int lfs_file_truncate_(lfs_t *lfs, lfs_file_t *file, lfs_off_t size) {
    ...
    if (size < oldsize) {
        ...
        while (file->pos < size) {
            uint8_t buffer = 0;
            res = lfs_file_write_(lfs, file, &buffer, 1);
            if (res < 0) {
                return (int)res;
            }
        }
...
static lfs_stag_t lfs_fs_parent(lfs_t *lfs, const lfs_block_t pair[2],
    ...
    while (!lfs_pair_isnull(parent->tail)) {
        ...
        struct lfs_fs_parent_match match = {
            lfs, {pair[0], pair[1]}
        };
