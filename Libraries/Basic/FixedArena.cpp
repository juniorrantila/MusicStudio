...
C_API FixedArena fixed_arena_init(void* memory, u64 size)
{
    ...
    return {
        .allocator = allocator_init(dispatch),
        .base = (u8*)memory,
        .head = (u8*)memory,
        .end = ((u8*)memory) + size,
    };
...
u64 FixedArena::bytes_left() const { return fixed_arena_bytes_left(this); }
C_API u64 fixed_arena_bytes_left(FixedArena const* arena)
{
    return arena->end - arena->head;
}
