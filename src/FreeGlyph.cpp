...
void free_glyph_atlas_init(FreeGlyphAtlas *atlas, FT_Face face)
{
    ...
    for (int i = 32; i < 128; ++i) {
        ...
        atlas->metrics[i].tx = (f32) x / (f32) atlas->atlas_width;
...
f32 free_glyph_atlas_cursor_pos(const FreeGlyphAtlas *atlas, c_string text, usize text_size, Vec2f pos, usize col)
{
