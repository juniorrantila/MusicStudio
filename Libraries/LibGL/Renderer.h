...
static_assert(alignof(GLVertex) == 16);
static constexpr GLVertexAttrib gl_vertex_attribs[] = {
    (GLVertexAttrib){
        .count = 4,
        .offset = OFFSET_OF(GLVertex, color),
    },
    (GLVertexAttrib){
        .count = 4,
        .offset = OFFSET_OF(GLVertex, bits),
    },
    (GLVertexAttrib){
        .count = 2,
        .offset = OFFSET_OF(GLVertex, position),
    },
    (GLVertexAttrib){
        .count = 2,
        .offset = OFFSET_OF(GLVertex, uv0),
    },
    (GLVertexAttrib){
        .count = 2,
        .offset = OFFSET_OF(GLVertex, uv1),
    },
};
...
typedef struct GLRenderer {
    ...
    GLShaderID shader(GLShaderSource);
...
static_assert(sizeof(GLRenderer) < 700 * KiB);

C_API void gl_clear(GLRenderer*, v4 color);

C_API GLShaderID gl_shader(GLRenderer*, GLShaderSource source);
