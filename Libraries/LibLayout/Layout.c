...
typedef struct Layout {
    NVGcontext* nvg;
    void (*on_error)(void* user, char const* message, u64 size);
    void* on_error_user;
...
Layout* layout_create(void* on_error_user, void(*on_error)(void* user, char const*, u64))
{
...
int layout_add_typeface(Layout* layout, c_string name, u8 const* data, u64 data_size)
{
    return nvgCreateFontMem(layout->nvg, name, (u8*)data, data_size, 0);
}

void layout_update_scroll(Layout* layout, v2 scroll, f32 delta_time)
{
    (void)layout;
...
void layout_set_pointer_state(Layout* layout, v2 position, bool left_down)
{
    (void)layout;
    Clay_SetPointerState((Clay_Vector2){ position.x, position.y }, left_down);
}

void layout_set_size(Layout* layout, v2 size, f32 pixel_ratio)
{
    (void)layout;
    layout->width = size.x;
