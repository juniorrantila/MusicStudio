...
static_assert(OFFSET_OF(TarHeader, path) == 0);
static_assert(OFFSET_OF(TarHeader, mode) == 100);
static_assert(OFFSET_OF(TarHeader, owner) == 108);
static_assert(OFFSET_OF(TarHeader, group) == 116);
static_assert(OFFSET_OF(TarHeader, file_size) == 124);
static_assert(OFFSET_OF(TarHeader, mtime) == 136);
static_assert(OFFSET_OF(TarHeader, checksum) == 148);
static_assert(OFFSET_OF(TarHeader, link_indicator) == 156);
static_assert(OFFSET_OF(TarHeader, link_path) == 157);
...
static void header_init(char const* path, u64 path_size, u64 content_size, TarHeader* header)
{
...
C_API e_tar tar_add(Tar* tar, c_string path, void const* content, u64 content_size)
{
    return tar_add2(tar, path, strlen(path), content, content_size);
}

C_API e_tar tar_add2(Tar* tar, char const* path, u64 path_size, void const* content, u64 content_size)
{
